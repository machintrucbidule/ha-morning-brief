...
- **HACS Validate workflow has 2 outstanding failures** on initial push (`commit 80b0050`), both expected at this stage:
  - `<Validation topics>`: the GitHub repo `machintrucbidule/ha-morning-brief` has no topics. Fix via `gh repo edit --add-topic home-assistant,hacs,integration,...` — does not require a commit.
  - `<Validation brands>`: HACS requires either `custom_components/morning_brief/brand/icon.png` (+ `dark_icon.png`) committed in the repo OR a PR into the `home-assistant/brands` repo registering this integration. Defer to Phase 11/12 release prep — not blocking dev work on Phase 2+.
- **Same fixes on the card repo** (`morning-brief-card`): hacs.json country code corrected. CI workflow `lint-build` fails because `package.json` is a stub (no dependencies declared) — expected; Phase 10 will fix that.

CI status on `machintrucbidule/ha-morning-brief` after fixes:
- ✅ Tests (pytest, 9/9 store tests green)
- ✅ Lint (ruff + mypy --strict)
- ❌ HACS Validate (brands + topics — see above)
...
- 2026-05-15 — Initialized two git repos and pushed: `home-assistant-morning-brief` → https://github.com/machintrucbidule/ha-morning-brief (2 commits: `chore: bootstrap memory files and repo scaffolding`, `feat: foundation skeleton`); `morning-brief-card` → https://github.com/machintrucbidule/-ha-morning-brief-card (1 commit: scaffolding). Local git user.email set to `ivan.calmels@gmail.com` per-repo. Triggered first CI run; iterated 2 fix commits to get Tests + Lint green: `fix(ci): pytest-asyncio auto mode, mypy cast, hacs country` (created pyproject.toml with `asyncio_mode = "auto"`, dropped a redundant `cast` in `store.py`, removed `'EN'` from `country` in both repos' hacs.json since that field is ISO 3166-1 alpha-2), then `fix(tests): respect MIN_RETENTION=5 in rotation tests` (two tests asserted post-rotation lengths that ignored the clamp). Section 38 ritual fully applied for Phase 1.
